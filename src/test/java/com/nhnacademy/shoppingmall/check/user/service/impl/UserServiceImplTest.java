...
class UserServiceImplTest {
    ...
    User testUser =
            new User("nhnacademy-test-user", "nhn아카데미", "nhnacademy-test-password", "19900505", User.Auth.ROLE_USER,
                    100_0000, LocalDateTime.now(), LocalDateTime.now());
    ...
    void doLogin() {
        Mockito.when(userRepository.findByUserIdAndUserPassword(anyString(), anyString()))
    ...
    void doLogin_fail() {
        ...
        Assertions.assertThrows(UserNotFoundException.class,
                () -> userService.doLogin(testUser.getUserId(), testUser.getUserPassword()));
        Mockito.verify(userRepository, Mockito.times(1)).findByUserIdAndUserPassword(anyString(), anyString());
    }
