...
<div class="btn-group" style="margin-bottom: 25px">
    ...
    <ul class="dropdown-menu">
        <c:forEach items="${categoryList}" var="category">
            <li><a class="dropdown-item" href="/index.do?categoryId=${category.categoryId}">${category.categoryName}</a>
            </li>
        </c:forEach>
    </ul>
</div>
        ...
        <ul class="pagination justify-content-center">
            ...
            <c:choose>
                <c:when test="${categoryId eq null}">
                    <c:set var="url" value="/index.do?page="/>
                </c:when>
                <c:otherwise>
                    <c:set var="url" value="/index.do?categoryId=${categoryId}&page="/>
                </c:otherwise>
            </c:choose>
            ...
            <c:forEach begin="1" end="${totalPage}" varStatus="page">
                <li class="page-item"><a class="page-link"
                                         href="${url}${page.index}">${page.index}</a></li>
            </c:forEach>
            <%-- Pagination >> --%>
